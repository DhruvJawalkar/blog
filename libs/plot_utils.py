...
def get_result_on_test_image(pred_bbox, pred_cat_id, conf, get_label_fn, im):
    w,h = im.size
    ...
    add_text_to_subplot(ax, (pred_bbox[0], pred_bbox[1]), 'Pred:'+get_label_fn(tensor_to_scalar(pred_cat_id[0])) +' ('+ "%.2f"%(conf*100) +'%)', size='medium')
    hide_subplot_axes(ax)

    #canvas = FigureCanvas(fig)
    #output = io.BytesIO()
    #canvas.print_png(output)
    #plt.savefig(output, format='png', bbox_inches='tight', pad_inches=0, frameon=False, transparent=True)
